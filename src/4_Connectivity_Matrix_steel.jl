...
function Build_Connect(Elements, Positions, Size)
    ...
    nel = size(Unique_Connections, 1) + size(steel_uniques, 1) + size(steel_bond_uniques, 1)
    ...
    @inbounds for i = 1:nel
        if i <= size(Unique_Connections, 1)
            ...
            Unique_Connections_Elements[i] = UniqueElementsStructure(Elements.Projected[Elements_ID, :], Elements.Non_Projected[Elements_ID, :], Unique_Connections[i, 1], Unique_Connections[i, 2], Elements.Tet[Elements_ID], Array{Int64,1}(undef, count(Elements_ID)))
            Connection_Area = 0.0
            Connection_Centroid = [0.0 0.0 0.0]
        ...
        elseif size(Unique_Connections, 1) < i <= size(Unique_Connections, 1) + size(steel_uniques, 1)                                              # Indices for all the appearances of current Unique Connection
            Unique_Connections_Elements[i] = UniqueElementsStructure(Array{Float64,2}(undef, 1, 1), Array{Float64,2}(undef, 1, 1), steel_uniques[i-size(Unique_Connections, 1)][1], steel_uniques[i-size(Unique_Connections, 1)][2], Array{Int64,1}(undef, 1), [pi * diameter_S^2 / 4])
            id_1 = steel_uniques[i-size(Unique_Connections, 1)][1]
            id_2 = steel_uniques[i-size(Unique_Connections, 1)][2]
            A = nodes_of_steel_coor_ma[id_1-n_nodes, :]                                                      # Node A of the Strut
            B = nodes_of_steel_coor_ma[id_2-n_nodes, :]                                                      # Node B of the Strut
            len = norm(B - A)                                                            # Length of the Strut
            Agg_1 = len                                                         # Diameter of the 1st Aggregate
            Agg_2 = len                                                         # Diameter of the 2nd Aggregate
            len_agg = (Agg_1 + Agg_2) / 2                                                # Total Length of Aggregate Included in the Strut
            len_m = len - len_agg
            Connection_Centroid = (A + B) / 2                                                   # Effective Matrix Length
            uv_1 = (B - A) / norm(B - A)                                                     # Unit Vector normal to the strut
            if uv_1[3] == 0 && uv_1[1] == uv_1[2]                                      # Choose between the two previously defined vectors
                perp = [uv_1[2] - uv_1[3], uv_1[1], uv_1[1]]
            else
            ...
            uv_3 = cross(uv_1, uv_2)
            dofs = collect([gdl_n*id_1-(gdl_n-1):gdl_n*id_1; gdl_n*id_2-(gdl_n-1):gdl_n*id_2])
            Connect[i] = ConnectivityVector(id_1, id_2, A, B, pi * diameter_S^2 / 4, vec(Connection_Centroid), [uv_1; uv_2; uv_3], len, len_agg, len_m, dofs)
        elseif i > size(Unique_Connections, 1) + size(steel_uniques, 1)                                              # Indices for all the appearances of current Unique Connection
            Unique_Connections_Elements[i] = UniqueElementsStructure(Array{Float64,2}(undef, 1, 1), Array{Float64,2}(undef, 1, 1), steel_bond_uniques[i-(size(Unique_Connections, 1)+size(steel_uniques, 1))][1], steel_bond_uniques[i-(size(Unique_Connections, 1)+size(steel_uniques, 1))][2], Array{Int64,1}(undef, 1), Array{Float64,1}(undef, 1))
            id_1 = steel_bond_uniques[i-(size(Unique_Connections, 1)+size(steel_uniques, 1))][1]
            id_2 = steel_bond_uniques[i-(size(Unique_Connections, 1)+size(steel_uniques, 1))][2]
            A = all_nodes_with_steel[id_1, :]                                                      # Node A of the Strut
            B = all_nodes_with_steel[id_2, :]                                                      # Node B of the Strut
            len = norm(B - A)
            if id_1 <= n_nodes                                                           # Length of the Strut
                Agg_1 = Size[id_1]                                                         # Diameter of the 1st Aggregate
                Agg_2 = diameter_S
                Connection_Centroid = B + (A - B) / len * diameter_S / 2
            else                                                     # Diameter of the 1st Aggregate
                Agg_1 = diameter_S
                Agg_2 = Size[id_2]
                Connection_Centroid = A + (B - A) / len * diameter_S / 2
    ...
    Unique_Connections = cat(Unique_Connections, steel_uniques_ma, steel_bond_uniques_ma; dims=1)
...
function Compatibility_Matrix(Connect)
    ...
    @inbounds for i = 1:nel
        ...
        A1 = [1.0 0.0 0.0 0.0 Center[3]-I[3] I[2]-Center[2]; 0.0 1.0 0.0 I[3]-Center[3] 0.0 Center[1]-I[1]; 0.0 0.0 1.0 Center[2]-I[2] I[1]-Center[1] 0.0]    # B_1, is 3*6
        A2 = [1.0 0.0 0.0 0.0 Center[3]-J[3] J[2]-Center[2]; 0.0 1.0 0.0 J[3]-Center[3] 0.0 Center[1]-J[1]; 0.0 0.0 1.0 Center[2]-J[2] J[1]-Center[1] 0.0]    # B_2, is 3*6
