...
# 3 = Imposed Displacement
    ...
    F_residual1 = F_residual[output_direction_load.= 1,1:step_interval:end]
    displace_12 = displace[output_direction_dis.= 2, 1:step_interval:end]

    Force_disp_collect_load = mapreduce(permutedims, vcat, [sum(F_residual1[:,i]) for i in axes(F_residual1,2)])
    Force_disp_collect_dis = mapreduce(permutedims, vcat, [sum(displace_12[:,i]) for i in axes(displace_12,2)])
